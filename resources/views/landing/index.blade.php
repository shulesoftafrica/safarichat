...
<body class="font-inter bg-white">
    ...
    <section id="features" class="py-20 bg-light">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 class="text-4xl font-bold text-center text-dark mb-16">{{ $content['skills']['title'] ?? 'My Core Sales Skills' }}</h2>
            
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                @if(isset($content['skills']['sales_conversations']))
                <div class="bg-white rounded-lg p-6 shadow-sm">
                    <h3 class="text-xl font-bold text-primary mb-4">{{ $content['skills']['sales_conversations']['title'] }}</h3>
                ...
                @if(isset($content['skills']['lead_management']))
                <div class="bg-white rounded-lg p-6 shadow-sm">
                    <h3 class="text-xl font-bold text-primary mb-4">{{ $content['skills']['lead_management']['title'] }}</h3>
                    <ul class="space-y-2 text-gray-600">
                ...
                @if(isset($content['skills']['campaigns']))
                <div class="bg-white rounded-lg p-6 shadow-sm">
                    <h3 class="text-xl font-bold text-primary mb-4">{{ $content['skills']['campaigns']['title'] }}</h3>
                    <ul class="space-y-2 text-gray-600">
                ...
                @if(isset($content['skills']['collaboration']))
                <div class="bg-white rounded-lg p-6 shadow-sm">
                    <h3 class="text-xl font-bold text-primary mb-4">{{ $content['skills']['collaboration']['title'] }}</h3>
                    <ul class="space-y-2 text-gray-600">
                ...
                @if(isset($content['skills']['whatsapp_management']))
                <div class="bg-white rounded-lg p-6 shadow-sm">
                    <h3 class="text-xl font-bold text-primary mb-4">{{ $content['skills']['whatsapp_management']['title'] }}</h3>
                    <ul class="space-y-2 text-gray-600">
    ...
    <section id="demo" class="py-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-bold text-dark mb-4">{{ $content['demo']['title'] ?? 'See Me In Action' }}</h2>
                <p class="text-xl text-gray-600 max-w-3xl mx-auto">{{ $content['demo']['description'] ?? 'Interactive chat where you can talk to me directly' }}</p>
            </div>
            ...
            <div class="max-w-2xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
                ...
                <div id="chatMessages" class="h-96 overflow-y-auto p-4 bg-gray-50">
                    <div class="chat-bubble-ai">
                        {{ $content['demo']['welcome'] ?? 'Hi! I\'m your AI Sales Agent. Ask me anything about how I can help grow your business!' }}
                    </div>
                </div>
    ...
    <section id="pricing" class="py-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-bold text-dark mb-4">{{ $content['pricing']['header'] }}</h2>
                <p class="text-xl text-gray-600 max-w-3xl mx-auto">{{ $content['pricing']['subheader'] }}</p>
            </div>
            
            <div class="grid md:grid-cols-3 gap-8">
                <!-- Starter Plan -->
                <div class="pricing-card bg-white rounded-lg p-8 border-2 border-gray-200 relative">
                    <h3 class="text-2xl font-bold text-dark mb-2">{{ $content['pricing']['starter_plan'] ?? 'Starter Plan' }}</h3>
                    <div class="text-4xl font-bold text-primary mb-4">
                        {{ $pricingData['symbol'] }}{{ number_format($pricingData['plans']['starter']['price'], 2) }}
                        <span class="text-lg font-normal text-gray-600">{{ $content['pricing']['per_month'] ?? '/month' }}</span>
                    </div>
                    <p class="text-gray-600 mb-6">{{ $content['pricing']['includes'] ?? 'Includes' }} {{ $pricingData['plans']['starter']['messages'] }} {{ $content['pricing']['ai_messages'] ?? 'AI messages' }}</p>
                    <div class="text-sm text-gray-500 mb-6">
                        {{ $content['pricing']['effective_rate'] ?? 'Effective rate' }}: {{ $pricingData['symbol'] }}{{ $pricingData['plans']['starter']['rate'] }} {{ $content['pricing']['per_message'] ?? 'per message' }}
                    </div>
                    <button class="w-full bg-gray-600 text-white py-3 rounded-lg font-semibold hover:bg-gray-700 transition-colors">
                        {{ $content['pricing']['get_started'] ?? 'Get Started' }}
                    </button>
                    <p class="text-sm text-gray-500 mt-4">{{ $content['pricing']['perfect_for'] ?? 'Perfect for' }}: Small businesses, startups</p>
                </div>
                
                <!-- Pro Plan -->
                <div class="pricing-card bg-white rounded-lg p-8 border-2 border-primary relative transform scale-105">
                    <div class="absolute -top-3 left-1/2 transform -translate-x-1/2">
                        <span class="bg-secondary text-dark px-4 py-1 rounded-full text-sm font-semibold">{{ $content['pricing']['most_popular'] ?? 'Most Popular' }}</span>
                    </div>
                    <h3 class="text-2xl font-bold text-dark mb-2">{{ $content['pricing']['pro_plan'] ?? 'Pro Plan' }}</h3>
                    <div class="text-4xl font-bold text-primary mb-4">
                        {{ $pricingData['symbol'] }}{{ number_format($pricingData['plans']['pro']['price'], 2) }}
                        <span class="text-lg font-normal text-gray-600">{{ $content['pricing']['per_month'] ?? '/month' }}</span>
                    </div>
                    <p class="text-gray-600 mb-6">{{ $content['pricing']['includes'] ?? 'Includes' }} {{ $pricingData['plans']['pro']['messages'] }} {{ $content['pricing']['ai_messages'] ?? 'AI messages' }}</p>
                    <div class="text-sm text-gray-500 mb-6">
                        {{ $content['pricing']['effective_rate'] ?? 'Effective rate' }}: {{ $pricingData['symbol'] }}{{ $pricingData['plans']['pro']['rate'] }} {{ $content['pricing']['per_message'] ?? 'per message' }}
                    </div>
                    <button class="w-full bg-primary text-white py-3 rounded-lg font-semibold hover:bg-primary/90 transition-colors">
                        {{ $content['pricing']['get_started'] ?? 'Get Started' }}
                    </button>
                    <p class="text-sm text-gray-500 mt-4">{{ $content['pricing']['perfect_for'] ?? 'Perfect for' }}: Growing businesses, schools</p>
                </div>
                
                <!-- Enterprise Plan -->
                <div class="pricing-card bg-white rounded-lg p-8 border-2 border-gray-200 relative">
                    <div class="absolute -top-3 left-1/2 transform -translate-x-1/2">
                        <span class="bg-green-500 text-white px-4 py-1 rounded-full text-sm font-semibold">{{ $content['pricing']['best_value'] ?? 'Best Value' }}</span>
                    </div>
                    <h3 class="text-2xl font-bold text-dark mb-2">{{ $content['pricing']['enterprise_plan'] ?? 'Enterprise Plan' }}</h3>
                    <div class="text-4xl font-bold text-primary mb-4">
                        {{ $pricingData['symbol'] }}{{ number_format($pricingData['plans']['enterprise']['price'], 2) }}
                        <span class="text-lg font-normal text-gray-600">{{ $content['pricing']['per_month'] ?? '/month' }}</span>
                    </div>
                    <p class="text-gray-600 mb-6">{{ $content['pricing']['includes'] ?? 'Includes' }} {{ $pricingData['plans']['enterprise']['messages'] }} {{ $content['pricing']['ai_messages'] ?? 'AI messages' }}</p>
                    <div class="text-sm text-gray-500 mb-6">
                        {{ $content['pricing']['effective_rate'] ?? 'Effective rate' }}: {{ $pricingData['symbol'] }}{{ $pricingData['plans']['enterprise']['rate'] }} {{ $content['pricing']['per_message'] ?? 'per message' }}
                    </div>
                    <button class="w-full bg-primary text-white py-3 rounded-lg font-semibold hover:bg-primary/90 transition-colors">
                        {{ $content['pricing']['get_started'] ?? 'Get Started' }}
                    </button>
                    <p class="text-sm text-gray-500 mt-4">{{ $content['pricing']['perfect_for'] ?? 'Perfect for' }}: High-volume organizations</p>
                </div>
            </div>
            
            <p class="text-center text-gray-600 mt-8 max-w-3xl mx-auto">
                {{ $content['pricing']['footer_note'] }}
            </p>
            <p class="text-center text-sm text-gray-500 mt-4">
                {{ $content['pricing']['overage_rate'] ?? 'Additional messages at' }} {{ $pricingData['symbol'] }}{{ $pricingData['plans']['overage_rate'] }} {{ $content['pricing']['per_message'] ?? 'per message' }}
            </p>
        </div>
    </section>
    ...
    <section id="contact" class="py-20">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-bold text-dark mb-4">{{ $content['contact_form']['title'] ?? 'How to Get Started Working With Me' }}</h2>
