...
function get_message() {
     ...
     *
     */
    if ((strlen($from) > 0) AND ( strlen($message) > 0) AND ( strlen($sent_timestamp) > 0 )
            AND ( strlen($message_id) > 0)) {
        /* The screte key set here is 123456. Make sure you enter
         * that on SMSsync.
         */
        if (( $secret == '12345dikodiko')) {
            $success = true;
        } else {
...
function get_sms_delivery_report() {
    if ($_GET['task'] === 'result' AND $_GET['secret'] === '12345dikodiko') {
